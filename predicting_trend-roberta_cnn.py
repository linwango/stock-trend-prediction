...
class BERT_NN(nn.Module):
    def __init__(self, max_len=MAXLEN, batch_size=BATCH_SIZE):
        ...
        self.conv1 = torch.nn.Conv1d(self.max_len, 16, 3, stride=1, padding=1, dilation=1, groups=1, bias=True,
                                     padding_mode='zeros')
        self.conv2 = torch.nn.Conv1d(16, self.cnn_output_size, 3, stride=1, padding=1, dilation=1, groups=1, bias=True,
                                     padding_mode='zeros')

        # Linear
        self.fc = nn.Linear(self.hidden_size * self.cnn_output_size, 2)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        #    input_ids/attention_mask dimensions - [-1, sequence_length=64, hidden_size=768]
        #    os dimensions -  [-1, sequence_length=64, hidden_size=768]
        ...
        # Mean
        x = torch.stack([hs[-1], hs[-2], os])  # [-1, 3, sequence_length=64, hidden_size=768]
        x = torch.mean(x, 0)  # [-1, sequence_length=64, hidden_size=768]

        # CNN
        x = self.conv1(x)  # [-1, 16, hidden_size=768]
        x = self.conv2(x)  # [-1, 2, hidden_size=768]

        # Dropout
        x = self.dropout(x)

        # Linear
        x = x.reshape(num_sample, self.hidden_size * self.cnn_output_size)
        x = self.fc(x)
